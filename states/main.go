...
func generateResourceMap(organization string, provider string) {
	...
	serializedMap, _ := json.Marshal(m)
	resourceFileName := fmt.Sprintf("%s_resouce.json", provider)
	os.Remove(resourceFileName)
	ioutil.WriteFile(resourceFileName, []byte(serializedMap), 0644)
}
...
func main() {
	...
	cachePtr := flag.Bool("cache", true, "use cache?")
	organizationPtr := flag.String("organization", "terraform-providers", "the github org to fetch provider from")

	if !*cachePtr {
		generateResourceMap(*organizationPtr, *providerPtr)
	}
	resourceList := listUniqueResources(*statePtr, *providerPtr)
	createPolicyDocument(resourceList, *providerPtr)
}
